...
__1.__ 3 times (30 minutes, 40 minutes, 50 minutes)

__2.__ 5 replicates for each treatment (I like pizza)
...
__It would not be a valid randomization, as it doesn't constrain the replicates to be 5 for each treatment level__
...
*The probabilities would be:* `perm 1 =` $\left( \frac{1}{2}\right) ^ 4$, `perm 2 =` $\left( \frac{1}{2}\right) ^ 3$, `perm 2 =` $\left( \frac{1}{2}\right) ^ 5$. So not a valid randomization scheme.
...
```{r}
data.frame(EUs = sample(15), treatment = rep(c(1,2,3), each = 5)) %>% kable()
```
